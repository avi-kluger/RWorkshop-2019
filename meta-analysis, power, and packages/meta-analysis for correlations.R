...
# Run function metacor from "meta" to meta analyze correlations and 
...
pwr.t.test(d = .50, sig.level = .05, power = .80, 
           type = c("two.sample"), alternative = c("greater"))
pwr.r.test(r = .2, power = .80, sig.level = .05, alternative = "two.sided")
